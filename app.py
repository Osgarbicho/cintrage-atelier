...
# =========================================================
st.set_page_config(
    page_title="CintragePro V5.0 - DXF",
    layout="wide",
    page_icon="🏭",
...
def solve_engine_v5(pts_x, pts_y, W_total, target_min, num_arcs, symmetric=False, weights=None):
    ...
    def build_curve(x_vals, params):
        ...
        if symmetric:
            n = num_arcs
            for i in range(n // 2): Rs[n-1-i] = Rs[i]
            for i in range((n-1) // 2): ths[n-2-i] = ths[i]
        ...
        return np.maximum(0, y_res), transitions_x.tolist(), Rs, ths

    def objective(params):
        if params[0] < 0.05 or params[0] > 1.65: return 1e12
        for i in range(1, len(params)):
            if i % 2 == 1 and params[i] < 50: return 1e12
            if i % 2 == 0 and params[i] < 0.03: return 1e12
        
        try: y_calc, trans, Rs, ths = build_curve(pts_x, params)
        except Exception: return 1e12
        
        if any(t <= 0 for t in trans): return 1e12
        for i in range(len(trans) - 1):
            if trans[i] >= trans[i+1]: return 1e12
        ...
        for r, th in zip(Rs, ths):
            L = r * th
            if L < target_min: pen_len += ((target_min - L) / target_min) ** 2 * 5000
        ...
        for k, tx in enumerate(trans):
            ...
            slope_diff = abs((y_r[0] - y_l[0]) / 0.2)
    ...
    def make_bounds(n_params):
        ...
        for i in range(n_params):
            if i == 0: bnds.append((0.05, 1.65))
            elif i % 2 == 1: bnds.append((100, W_total * 10))
            else: bnds.append((0.05, 1.8))
        return bnds

    n_params = 1 + num_arcs * 2 - 1
    bnds = make_bounds(n_params)
    
    with st.spinner("⚡ Phase 1 : Recherche globale (Differential Evolution)..."):
        de_result = differential_evolution(
            objective, bounds=bnds, maxiter=600, tol=1e-9, seed=42, 
            workers=1, mutation=(0.5, 1.2), recombination=0.85, 
            popsize=18, init='latinhypercube', polish=False
        )

    with st.spinner("🔬 Phase 2 : Affinage local (L-BFGS-B)..."):
        fine_result = minimize(
            objective, de_result.x, bounds=bnds, method='L-BFGS-B',
    ...
    return {
        "x_plot": x_plot, "y_plot": y_plot, "trans": trans, "Rs": Rs, "Ls": Ls, "ths": ths,
        "err_mean": err_mean, "err_max": err_max, "err_pts": err_pts.tolist(),
        "width": float(W_total), "alpha": float(np.degrees(best_params[0])),
        "confidence": confidence, "de_score": float(de_result.fun), "fine_score": float(fine_result.fun),
        "params": best_params.tolist(), "pts_x": pts_x.tolist(), "pts_y": pts_y.tolist(),
        "num_arcs": num_arcs, "timestamp": datetime.now().strftime("%H:%M:%S"),
...
def make_main_chart(res, compare_res=None, pts_x=None, pts_y=None):
    fig = go.Figure()
    x_plot, y_plot, trans, num_arcs, W = np.array(res["x_plot"]), np.array(res["y_plot"]), res["trans"], res["num_arcs"], res["width"]
    boundaries = [0.0] + trans + [W]
    
    for i in range(num_arcs):
        fig.add_vrect(x0=boundaries[i], x1=boundaries[i+1], fillcolor=COLORS[i % 5], opacity=0.03, layer="below", line_width=0)
    
    if compare_res:
        fig.add_trace(go.Scatter(x=compare_res["x_plot"], y=compare_res["y_plot"], mode='lines', name='Comparaison', line=dict(color='#505060', width=2, dash='dash'), opacity=0.6))
    
    for i in range(num_arcs):
        mask = (x_plot >= boundaries[i]) & (x_plot <= boundaries[i+1])
        fig.add_trace(go.Scatter(x=x_plot[mask], y=y_plot[mask], mode='lines', name=f'Arc {i+1} — R={res["Rs"][i]:.0f}mm', line=dict(color=COLORS[i % 5], width=4)))
    
    for i, tx in enumerate(trans):
        fig.add_vline(x=tx, line_dash="dot", line_color="#505060", line_width=1, annotation_text=f"T{i+1} {tx:.0f}mm", annotation_font_color="#a0a8b8", annotation_font_size=11)
    
    if pts_x is not None:
        fig.add_trace(go.Scatter(x=pts_x, y=pts_y, mode='markers', name='Points relevés', marker=dict(color='#ffffff', size=10, symbol='x', line=dict(width=2))))

    fig.update_layout(
        paper_bgcolor='#111318', plot_bgcolor='#151820', font=dict(family="Barlow, sans-serif", color='#a0a8b8'),
        legend=dict(bgcolor='#1a1d24', bordercolor='#2a2d35', borderwidth=1, font=dict(size=11)),
        xaxis=dict(gridcolor='#1e2128', zerolinecolor='#2a2d35', title="X (mm)", title_font_color='#a0a8b8', tickfont_color='#a0a8b8'),
        yaxis=dict(gridcolor='#1e2128', zerolinecolor='#2a2d35', scaleanchor="x", scaleratio=1, title="Y (mm)", title_font_color='#a0a8b8', tickfont_color='#a0a8b8'),
        height=500, margin=dict(l=40, r=40, t=40, b=40),
    )
    return fig

def make_error_chart(res):
    pts_x, err_pts = res["pts_x"], res["err_pts"]
    colors = ['#40f090' if e < 5 else '#f0c040' if e < 15 else '#f04050' for e in err_pts]
    fig = go.Figure()
    fig.add_trace(go.Bar(x=pts_x, y=err_pts, marker_color=colors, name="Erreur (mm)", text=[f"{e:.1f}" for e in err_pts], textposition='outside', textfont_color='#a0a8b8'))
    ...
    fig.update_layout(paper_bgcolor='#111318', plot_bgcolor='#151820', font=dict(family="Barlow", color='#a0a8b8'), xaxis=dict(gridcolor='#1e2128', title="X (mm)", tickfont_color='#a0a8b8'), yaxis=dict(gridcolor='#1e2128', title="Erreur (mm)", tickfont_color='#a0a8b8'), height=300, margin=dict(l=40, r=40, t=30, b=40), showlegend=False)
    return fig
...
with st.sidebar:
    ...
    symmetric = st.checkbox("Profil symétrique", value=False, help="Force R1=Rn, θ1=θn-1... Réduit l'espace de recherche et améliore la convergence pour les profils symétriques.")
    
    st.markdown("**Pondération des points :**")
    weight_mode = st.radio("Mode de pondération", ["Uniforme", "Bords renforcés", "Manuel"], help="Bords renforcés : x4 aux extrémités. Manuel : modifiez le tableau de points.", label_visibility="collapsed")
...
with col_input:
    st.markdown("### 📁 CHARGER UN PLAN DXF (Optionnel)")
    fichier_dxf = st.file_uploader("Glissez le DXF pour remplacer les points", type=["dxf"])
    
    if fichier_dxf is not None:
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix='.dxf') as tmp:
                tmp.write(fichier_dxf.getvalue())
                tmp_path = tmp.name

            doc = ezdxf.readfile(tmp_path)
            msp = doc.modelspace()
            entites_courbes = msp.query('ARC SPLINE LWPOLYLINE LINE')

            if len(entites_courbes) > 0:
                points_bruts = []
                for entite in entites_courbes:
                    try:
                        chemin = make_path(entite)
                        pts_temp = []
                        for sommet in chemin.flattening(distance=1.0):
                            pts_temp.append([sommet.x, sommet.y])
                            
                        # Filtre anti-origine (taille > 20mm)
                        if len(pts_temp) > 1:
                            xs = [p[0] for p in pts_temp]
                            ys = [p[1] for p in pts_temp]
                            encombrement = (max(xs) - min(xs)) + (max(ys) - min(ys))
                            if encombrement > 20.0:
                                points_bruts.extend(pts_temp)
                    except Exception:
                        pass
                        
                if len(points_bruts) > 0:
                    points_array = np.array(points_bruts)
                    
                    # Recalage Zéro Automatique
                    points_array[:, 0] -= np.min(points_array[:, 0])
                    points_tries = points_array[points_array[:, 0].argsort()]
                    _, index_uniques = np.unique(np.round(points_tries[:, 0], 1), return_index=True)
                    points_propres = points_tries[index_uniques]
                    points_propres[:, 1] -= points_propres[0, 1]
                    
                    MAX_POINTS = 40
                    if len(points_propres) > MAX_POINTS:
                        indices_echantillon = np.linspace(0, len(points_propres) - 1, MAX_POINTS).astype(int)
                        points_finaux = points_propres[indices_echantillon]
                    else:
                        points_finaux = points_propres

                    default_pts = points_finaux.tolist()
                    st.success(f"✅ DXF Nettoyé ! {len(points_finaux)} points extraits et alignés sur le zéro.")
                else:
                    st.error("⚠️ Impossible d'extraire la géométrie. Chargement du profil...")
                    default_pts = PROFILS.get(profil_choice, PROFILS["Grand Profil (défaut)"]) if profil_choice != "Personnalisé" else PROFILS["Grand Profil (défaut)"]
            else:
                st.error("⚠️ Aucune géométrie trouvée. Chargement du profil...")
                default_pts = PROFILS.get(profil_choice, PROFILS["Grand Profil (défaut)"]) if profil_choice != "Personnalisé" else PROFILS["Grand Profil (défaut)"]
        ...
        if profil_choice != "Personnalisé" and PROFILS.get(profil_choice):
    ...
    if weight_mode == "Manuel":
        with st.expander("⚖️ Pondérations"):
            w_df = pd.DataFrame({"X (mm)": edited_df["X (mm)"].dropna().values, "Poids": [1.0] * len(edited_df.dropna())})
...
with col_main:
    if st.session_state.last_result:
        ...
        with tab1:
            fig = make_main_chart(res, compare_res=st.session_state.compare_result, pts_x=np.array(res["pts_x"]), pts_y=np.array(res["pts_y"]))
        ...
        with tab2:
            ...
            err_data = pd.DataFrame({"X (mm)": [f"{x:.0f}" for x in res["pts_x"]], "Y relevé": [f"{y:.1f}" for y in res["pts_y"]], "Erreur": [f"{e:.2f} mm" for e in res["err_pts"]], "État": ["✅" if e < 5 else "⚠️" if e < 15 else "❌" for e in res["err_pts"]]})
        ...
        with tab3:
            if st.session_state.history:
                for h in reversed(st.session_state.history[-5:]):
                    ...
                    st.markdown(f"<div style='background:#1a1d24;border:1px solid #2a2d35;border-radius:3px;padding:0.6rem 1rem;margin-bottom:0.4rem;display:flex;justify-content:space-between;align-items:center;'><span style='color:#a0a8b8;font-family:Barlow Condensed,sans-serif;font-size:0.85rem;'>{h['timestamp']} · {h['num_arcs']} arcs</span><span style='color:#e8e0d0;font-family:Share Tech Mono,monospace;font-size:0.9rem;'>Ø{h['err_mean']:.1f}mm</span><span style='color:{conf_col};font-family:Share Tech Mono,monospace;'>{h['confidence']}%</span></div>", unsafe_allow_html=True)
                if st.button("📌 Utiliser comme référence"):
        ...
        st.markdown("<div style='height:460px;display:flex;align-items:center;justify-content:center;border:1px dashed #2a2d35;border-radius:4px;'><div style='text-align:center;'><div style='font-size:3rem;'>⚙️</div><div style='color:#505060;font-family:Barlow Condensed,sans-serif;font-size:1.2rem;letter-spacing:2px;margin-top:1rem;'>LANCEZ UN CALCUL</div><div style='color:#404050;font-family:Barlow,sans-serif;font-size:0.85rem;margin-top:0.5rem;'>Entrez vos points et cliquez sur Lancer le calcul</div></div></div>", unsafe_allow_html=True)

# ─── CALCUL ───
if run_calc:
    df = edited_df.dropna().astype(float).sort_values("X (mm)")
    if len(df) < 3:
        st.warning("⚠️ Attention : Vous devez renseigner au moins 3 points géométriques pour calculer des arcs.")
    else:
        ...
        st.session_state.history.append({"timestamp": res["timestamp"], "num_arcs": nb_rayons, "err_mean": res["err_mean"], "confidence": res["confidence"]})
        st.session_state.last_nb_rayons = nb_rayons
        st.session_state.last_target = target_val
        st.rerun()

# ─── AFFICHAGE RÉSULTATS (piloté par session_state) ───
if st.session_state.last_result:
    res = st.session_state.last_result
    ...
    for i in range(disp_nb_rayons):
        L = res["Ls"][i]
        R = res["Rs"][i]
        status = "✅ OK" if L >= disp_target else ("⚠️ Court" if L >= disp_target * 0.8 else "❌ Trop Court")
        rows.append({"Zone": f"Arc {i+1}", "Rayon (mm)": round(R, 1), "Dév. L (mm)": round(L, 1), "Cible (mm)": disp_target, "État": status})

    final_df = pd.DataFrame(rows)
    
    def color_status(row):
        if "✅" in row['État']: return ['background-color: #0d2010; color: #50d080'] * len(row)
        elif "⚠️" in row['État']: return ['background-color: #201500; color: #f0a040'] * len(row)
        else: return ['background-color: #200d0d; color: #e05050'] * len(row)

    st.dataframe(final_df.style.apply(color_status, axis=1).format({"Rayon (mm)": "{:.1f}", "Dév. L (mm)": "{:.1f}"}), use_container_width=True)
